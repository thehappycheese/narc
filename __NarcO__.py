...
class NarcO(object):
	"""Represents and ArcPy Object"""
	def __init__(self, path):
		"""Ensure path argument is valid, will throw error if arcpy.Describe() fails. The argument is coerced to str() so it can be a NarcO()"""
		if type(path) is NarcO:
			self._path = path._path
			self._desc = path._desc
		else:
			self._path = str(path)
			self._desc = None
	...
	def get_desc(self):
		if self._desc is None:
			if not os.path.exists(self._path):
				pass
				print("cannot describe if not exists")
	...
	def is_gdb(self):
		"""deprecated"""
		return self.filename()[-4:].lower() == ".gdb"

	def is_in_gdb(self):
		"""deprecated"""
		return True in map(lambda item:item[-4:].lower() == ".gdb",list(self.folder().split("\\")))
	
	def is_workspace(self):
		return self.get_desc().dataType == "Workspace"

	def is_feature_class(self):
		return self.get_desc().dataType == "FeatureClass"

	def gdb_and_feature_class_name(self):
		if self.is_gdb():
	...
	def append_path(self,new_path_part):
		'''in place append to path'''
		self._path = os.path.join(self._path,new_path_part)
		self._desc = None
		return self
	
	def from_append(self,arg_str_path):
		return NarcO(os.path.join(self._path, arg_str_path))
	...
	def arcpy_select_to(self, arg_output_path, where_clause):
		arg_output_path = NarcO(arg_output_path)
		thou_shalt(
			"Select from\n\t\t%s into\n\t\t%s\n\t\twhere %s"%(self.gdb_and_feature_class_name(),arg_output_path.gdb_and_feature_class_name(),where_clause),
	...
	def arcpy_select_from(self, arg_input_path, where_clause):
		arg_input_path = NarcO(arg_input_path)
		thou_shalt(
			"Select from\n\t\t%s INTO\n\t\t%s WHERE\n\t\t%s"%(arg_input_path.gdb_and_feature_class_name(),self.gdb_and_feature_class_name(),where_clause),
	...
	def arcpy_intersect_from(self, arg_list_paths, cluster_tolerance=None, output_type="INPUT"):
		...
		print_string = [[value,index+1] for index,value in list(enumerate(map(lambda item: NarcO(item).gdb_and_feature_class_name(),arg_list_paths)))] if auto_rank else map(str,arg_list_paths)
		thou_shalt(
			"Intersect [%s] with tollerance %s into %s"%(",".join(map(str,print_string)),str(cluster_tolerance),self.gdb_and_feature_class_name()),
	...
	def arcpy_clip_to(self, clip_features, arg_output_path, cluster_tolerance=None):
		...
		arg_output_path = NarcO(arg_output_path)
		clip_features = NarcO(clip_features)
		thou_shalt(
	...
	def arcpy_to_feature_class(self, arg_output_path, where_clause=None, field_mapping=None, config_keyword=None):
		arg_output_path = NarcO(arg_output_path)
		thou_shalt(
			"Create feature class from\n\t\t%s at \n\t\t%s"%(self.gdb_and_feature_class_name(), arg_output_path.gdb_and_feature_class_name()),
	...
	def arcpy_alter_field(self,field, new_field_name=None, new_field_alias=None, field_type=None, field_length=None, field_is_nullable=None, clear_field_alias=None):
		thou_shalt("Alter Field {} of {}".format(field, self.gdb_and_feature_class_name()),
	...
	def arcpy_rename(self,new_filename):
		...
		# TODO: fix this
		old_name = self.gdb_and_feature_class_name()
		old_path = self._path
		self._path = os.path.join(self.folder(), new_filename)
		thou_shalt("Rename\n\t\t{} to\n\t\t{}".format(old_name, self.gdb_and_feature_class_name()),
	...
	def arcpy_get_field_objects(self):
		...
		if __thou_shalt__.do_a_dry_run:
			return []
		return thou_shalt("Fetch field information from {}".format(self.gdb_and_feature_class_name()),
	...
	def arcpy_prefix_field_names(self, prefix):
		...
		thou_shalt("Prefix fields of \n\t\t{}\n\t\twith '{}'".format(self.gdb_and_feature_class_name(), safe_prefix),
			arcpy_call
		)
	...
	def arcpy_fill_column_with_value(self, field_name, arg_value):
		...
		thou_shalt("Populate column %s of %s with value %s"%(field_name, self.gdb_and_feature_class_name(), str(arg_value)),
		...
		def arcpy_call(field_name_list, lambda_process_row):
			...
			with arcpy.da.UpdateCursor(str(self), field_name_list) as cursor:  # pylint: disable=no-member
				for row in cursor:
					...
					cursor.updateRow(row)
		thou_shalt("Execute code on column list %s of %s" % (str(field_name_list), self.gdb_and_feature_class_name()),
			lambda: arcpy_call(field_name_list, lambda_process_row)
		)

	def arcpy_append_features(self, input_features_list):
		if type(input_features_list) is str or type(input_features_list) is NarcO:
			input_features_list = [input_features_list]
		input_features_list = map(NarcO, input_features_list)
		thou_shalt("Append features from  %s to %s" % (str([item.gdb_and_feature_class_name() for item in input_features_list]), self.gdb_and_feature_class_name()),
	...
	def arcpy_create_routes_linear_refernancing_from(self, arg_input_path, segment_identifier_column="NETWORK_ELEMENT", start_column="START_TRUE_DIST", end_column="END_TRUE_DIST"):
		arg_input_path = NarcO(arg_input_path)
		thou_shalt("Create Linear Referanceable Layer {} from {}".format(self.gdb_and_feature_class_name(), arg_input_path.gdb_and_feature_class_name()),
	...
	def arcpy_get_in_memory_version(self, where=None):
		...
		in_memory_path = NarcO("in_memory").from_append(self.filename())
		thou_shalt("Move to in_memory \n\t\t{} as {}".format(self.gdb_and_feature_class_name(), self.filename()),
	...
	def __add__(self, other):
		return NarcO.from_join(self._path, NarcO(other)._path)

	def __radd__(self, other):
		if type(other) is str:
			return other + self._path
		if type(other) is NarcO:
			return NarcO.from_join(other._path, self._path)
